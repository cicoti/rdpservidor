...
public class ServerConfigDialog extends JDialog {
    ...
    private JPanel buildHeaderPanel() {
        ...
        JLabel titleLabel = new JLabel("Configuração de Portas");
        titleLabel.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 20));
        titleLabel.setAlignmentX(LEFT_ALIGNMENT);

        JLabel subtitleLabel = new JLabel("Defina as portas locais utilizadas pelo servidor.");
    ...
    private JPanel buildInfoPanel() {
        ...
        JLabel infoLabel = new JLabel(
                "<html><span style='color:rgb(110,110,110);'>Alterações passam a valer após reiniciar a aplicação.</span></html>",
                SwingConstants.LEFT
        );
    ...
    private void onSave() {
        ...
        boolean changed =
                handshakePort != currentConfig.getHandshakePort()
        ...
        try {
            ServerConfig newConfig = new ServerConfig(handshakePort, controlPort);
            configManager.save(newConfig);

            if (changed) {
                JOptionPane.showMessageDialog(
                        this,
                        "Configuração salva com sucesso.\nReinicie a aplicação para aplicar as novas portas.",
                        "Configuração salva",
